...
var calculateScore, hotScore, isAllowedToUseSkill, naiveScore, nextLevel, notify, patch, prevLevels, updateScore, wilsonScore;
...
Meteor.methods({
  ...
  vote: function(data) {
    ...
    if (vote) {
      ...
      if (vote.up === true && data.up === false) {
        diff = -1;
      } else if (vote.up === false && data.up === true) {
        diff = 1;
      }
      if (diff) {
...
isAllowedToUseSkill = function(skill) {
  var isAllowed, setter, time, user;
  user = Meteor.user();
